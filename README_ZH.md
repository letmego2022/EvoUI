...
> 以下以“操作流程”形式展示核心页面，说明均提供中英文，并统一采用较小尺寸截图。
...
### 2）仪表盘（Dashboard）
**中文**：集中管理测试 Domain 与项目，同时提供 AI Agent 入口。  
...
**中文**：支持手动维护 PAGE / FUNCTION / FLOW / API 等多种类型。  
...
**中文**：AI BA 从 UI 元素中提取结构化信息并驱动用例生成，其它场景也可直接由 AI 生成。  
...
**中文**：AI 基于 API 类 User Story 自动生成脚本，支持对话式修改与主动优化，并保留偏好记忆。  
...
**中文**：执行 UI 自动化任务并输出报告，结合记忆机制持续提升稳定性。  
